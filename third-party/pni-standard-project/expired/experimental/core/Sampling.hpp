...
concept IsSamplingMethod = requires(T &sampler) {
  { sampler.next() } -> FloatingPoint_c; // 获取下一个采样值,这个采样值是线段参数方程的参数值，范围
  // 是 [0, 1]，0表示开始点，1表示结束点
  { sampler.lastStepSize() } -> FloatingPoint_c; // 上一次采样的步长
  { sampler.isEnd() } -> std::same_as<bool>;     // 采样结束了
};
...
struct SamplingUniform {
  ...
  __PNI_CUDA_MACRO__ bool setInfo(
      const p3df &point1, const p3df &point2, const size_t eventIndex, const basic::Image3DGeometry *__image3dSize,
      const basic::Vec2<p3df> *__roi = nullptr) {
    using basic::sgn;
    using basic::smallFloat;
    const p3df cryPoint1 = point1;
    const p3df cryPoint2 = point2;
    const auto cry2_cry1 = cryPoint2 - cryPoint1;
    ...
    constexpr auto SAMPLE_PRECISION = 1023;
    static_assert(SAMPLE_PRECISION > 0, "SAMPLE_PRECISION must be greater than 0");
    m_sampleOffset = ((eventIndex % SAMPLE_PRECISION) + 0.5) / T(SAMPLE_PRECISION);
    m_sampleIndex = 0;
    return true;
  }
