...
__PNI_CUDA_MACRO__ basic::Vec2<T> liang_barskey(
...
inline CrystalGeometry calculateCrystalGeometry(
    const Coordinate3D<float> &__detector, const DetectorGeometry &__geometry, uint16_t __crystalIndexU,
  ...
  result.position = basic::make_vec3<float>(__detector.position + directionU * (blockCenterU + crystalCenterU) +
                                            directionV * (blockCenterV + crystalCenterV));
  result.directionU = directionU * __geometry.crystalSizeU / 2;
  result.directionV = directionV * __geometry.crystalSizeV / 2;
...
inline CrystalGeometry calculateBlockGeometry(
  ...
  result.position =
      basic::make_vec3<float>(__detector.position + directionU * blockCenterU + directionV * blockCenterV);
