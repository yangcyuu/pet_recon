...
template <typename T>
concept ContractTransformAction2D = IsTransformAction2DDouble<T> || IsTransformAction2DFloat<T>;
...
template <typename T>
concept IsPoint = requires {
  typename T::value_type;
} && (std::same_as<T, basic::Vec2<typename T::value_type>> || std::same_as<T, basic::Vec3<typename T::value_type>>);
...
__PNI_CUDA_MACRO__ Point applyTransforms_impl(
    const Point &point, const Actions &...actions) {
...
template <typename InputValueType, typename OutputValueType, IsInterpolationMethod2D InterpolationMethod,
          typename... Actions>
  requires(IsTransformAction<Actions> && ...)
__PNI_CUDA_MACRO__ inline void transform2D_impl(
    const InputValueType *__input, OutputValueType *__output, const basic::Image2DGeometry &__ig,
    const basic::Image2DGeometry &__og, const InterpolationMethod &__ipMethod, int __x, int __y,
    const Actions &...negativedActions) {
  const auto pointOfOutput =
      basic::make_vec2<typename InterpolationMethod::value_type>(__x + .5, __y + .5) * __og.voxelSize + __og.imgBegin;
  const auto pointOfInput = applyTransforms_impl<false>(pointOfOutput, negativedActions...);
...
template <typename InputValueType, typename OutputValueType, IsInterpolationMethod2D InterpolationMethod,
          typename... Actions>
  requires(IsTransformAction<Actions> && ...)
__global__ void transform2D_kernel(
    const InputValueType *__input, OutputValueType *__output, basic::Image2DGeometry __ig, basic::Image2DGeometry __og,
  ...
  const auto pointOfOutput =
      basic::make_vec2<typename InterpolationMethod::value_type>(x + .5, y + .5) * __og.voxelSize + __og.imgBegin;
  const auto pointOfInput = applyTransforms_impl<false>(pointOfOutput, negativedActions...);
  __output[__og.at(x, y)] = __ipMethod(pointOfInput, __input, __ig);
}
template <typename InputValueType, typename OutputValueType, _impl::IsInterpolationMethod2D InterpolationMethod,
          typename... Actions>
  requires(_impl::IsTransformAction<Actions> && ...)
void transform2D(
    const InputValueType *__input, OutputValueType *__output, const basic::Image2DGeometry &__ig,
    const basic::Image2DGeometry &__og, const InterpolationMethod &__ipMethod, std::size_t maxThreadNum,
...
#pragma omp parallel for num_threads(std::max<int>(1, maxThreadNum))
  for (int __x = 0; __x < __og.voxelNum.x; __x++)
    for (int __y = 0; __y < __og.voxelNum.y; __y++)
      _impl::transform2D_impl(__input, __output, __ig, __og, __ipMethod, __x, __y, (-actions)...);
}
template <typename InputValueType, typename OutputValueType, _impl::IsInterpolationMethod2D InterpolationMethod,
          typename... Actions>
  requires(_impl::IsTransformAction<Actions> && ...)
void transform2D_cuda(
    const InputValueType *__input, OutputValueType *__output, const basic::Image2DGeometry &__ig,
    const basic::Image2DGeometry &__og, const InterpolationMethod &__ipMethod, std::size_t maxThreadNum,
    const Actions &...actions) {
  dim3 block(16, 16);
  dim3 grid((__og.voxelNum.x + block.x - 1) / block.x, (__og.voxelNum.y + block.y - 1) / block.y);
  _impl::transform2D_kernel<<<grid, block>>>(__input, __output, __ig, __og, __ipMethod, (-actions)...);
