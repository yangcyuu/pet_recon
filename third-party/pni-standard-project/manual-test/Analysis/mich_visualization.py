...
def plotFigure(__michPath, __view = -1):
    ...
    delay = np.fromfile("/home/ustc/Desktop/E180_20250826_WellCounter_PET4643_Slice4796/Data/result/random_add_sss.bin", dtype=np.float32, offset=0).reshape([RING_NUM*RING_NUM, int(CRYSTAL_NUM_ONE_RING / 2), int(CRYSTAL_NUM_ONE_RING - 1)])
    # scatter = np.fromfile("/media/lenovo/deadtime/dcf/1IQ/mich_3seg/scatter_2.mich", dtype=np.float32).reshape([104*104, 156, 311])
    ...
    # 在同一个图层上叠加两张图的结果并保存
    if isOneView == False:
        plt.plot(pr - de, label="prompts-delayed-sss")
        # plt.plot(sc, label="scatter")
    else:
        plt.plot(pr_sl[__view,:] - de_sl[__view,:], label="prompts-delayed-sss")
        # plt.plot(sc_sl[__view,:], label="scatter")

    # plt.ylim([0,2000000])
    plt.legend(fontsize=12)
    
    save_path = __michPath + '/prompt-delayed-sss.png'
    ...
    if isOneView == False:
        ...
        plt.plot(de, label="delay+sss")
    else:
        plt.plot(pr_sl[__view,:], label="prompt")
        plt.plot(de_sl[__view,:], label="delay+sss")

    # plt.ylim([0,2e6])
    plt.legend(fontsize=12)
    if isOneView == False:
        plt.title("All Views")
        save_path = __michPath + '/prompt&delaysss_all.png'
        if os.path.exists(save_path):
            os.remove(save_path)
        plt.savefig(__michPath + '/prompt&delaysss_all.png')  # 保存图形
    else:
        plt.title(f"View {__view}")
        save_path = __michPath + f'/prompt&delaysss_view_{__view}.png'
        if os.path.exists(save_path):
            os.remove(save_path)
        plt.savefig(__michPath + f'/prompt&delaysss_view_{__view}.png')  # 保存图形
